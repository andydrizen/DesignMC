...
BindGlobal("ScanLotsOfImproperLFK2Ns",function(n, L)
	...
	while true do
		...
		od;
		r:=ImproperDesignSpecialBlocks(m);
		if Size(r)>1 then
			Print("\n",i,": ---------\n\n ",m,"\n");
...
BindGlobal("ScanLotsOfImproperLFK2NsForLeastNumberOfTrails",function(B)
	...
	while true do
		...
		if Size(r)>1 then
			...
			tries:=MultisetDifference(SortListList(Cartesian([m.negatives[1][3]], MultisetDifference([m.vType[1]+1..m.vType[1]+m.vType[2]],[m.negatives[1][3]]))), SortListList(r));
			Print("tries = ",tries, "\n");
			...
			for j in tries do
				...
				deletableEdges:=get_blocks_containing_list(m, [badColour,m.negatives[1][1]]);
				...
				for k in deletableEdges do
					...
					if result>=tmp then
						...
						if zeroHunting = 1 then
							if tmp = 0 then
								if Size(ComponentsOfGraph(BlockDesign(m.v, MultisetDifference(m.blocks, get_blocks_containing_list(m, [m.negatives[1][1], badColour]))), [m.negatives[1][2], badColour]))=ComponentsOfGraph(m, [m.negatives[1][1], badColour]) then
...
BindGlobal("T1T2Pair",function(v,a,b)
	...
	od;
	blocks:=blocks+1;
	t1:=BlockDesign(v, SortListList(blocks));
	t1.negatives:=[];
	t1.improper:=false;
	t1.k:=[3];
	...
	od;
	blocks:=blocks+1;
	t2:=BlockDesign(v, SortListList(blocks));
	t2.negatives:=[];
	t2.improper:=false;
	t2.k:=[3];
	return [t1,t2];
...
BindGlobal("find_admissible",function(v)
	...
	for a in [0..v-1] do 
		for b in [0..v-1] do 
			if test_conditions(a,b,v)=1 and IsPartialSTS(T1T2Pair(v,a,b)[1])=1 and IsPartialSTS(T1T2Pair(v,a,b)[2])=1 then 
				Print("a=",a," b=",b,"\n"); 
			fi; 
		od;
	od; 
...
BindGlobal("FindBitrade",function(B)
	...
	B1:=ShallowCopy(B);
	B2:=OneStep(B1);
	T1:=GetMutableListList(B1.blocks);
	T2:=GetMutableListList(B2.blocks);
...
BindGlobal("FindBridgeToChordTrade",function(B)
	...
	while flag=0 do
		...
		for i in ImproperDesignSpecialBlocks(B2) do
			if IsChordedDG(B2, i) then
				flag:=1; 
				break;
			fi;
