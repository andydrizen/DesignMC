...
BindGlobal("IsCompleteLS",function(B,IsRowComplete,IsColumnComplete)
	local res,tmp,block,elt,elt2,i,flag,j;
	res:=[];

		
	for elt in [B.v/3*2+1..B.v] do
		tmp:=get_blocks_containing_list(B, [elt]);
		Add(res, [[],[],[],[]]);
		for block in tmp do
			if block[3] = elt then
				# get elts on down
				if not block[1]=B.v/3 then
					Add(res[Size(res)][1], get_blocks_containing_list(B,[block[1]+1,block[2]])[1][3]);
				fi;
				# get elts on right
				if not block[2]=B.v/3*2 then
					Add(res[Size(res)][2], get_blocks_containing_list(B,[block[1],block[2]+1])[1][3]);
				fi;
				# get elts on left
				if not block[2]=B.v/3+1 then
					Add(res[Size(res)][3], get_blocks_containing_list(B,[block[1],block[2]-1])[1][3]);
				fi;
				# get elts on up
				if not block[1]=1 then
					Add(res[Size(res)][4], get_blocks_containing_list(B,[block[1]-1,block[2]])[1][3]);
				fi;
			fi;
		od;
	od;
...
BindGlobal("ScanForCompleteLS",function(B,IsRowComplete,IsColumnComplete)
	...
	while true do
		i:=i+1;
		flag:=IsCompleteLS(B2,IsRowComplete,IsColumnComplete);
		if flag=1 then
			got:=0;
			for k in found do
...
BindGlobal("ScanForCompleteLSInList",function(List, IsRowComplete, IsColumnComplete)
