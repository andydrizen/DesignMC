...
BindGlobal("StinsonProper",function(n)
	...
	while Size(B.blocks) < n*(n-1)/6 do
		...
		z:=MultisetDifference(Random(MultisetDifference(livepairs2, [Sort2([y,x])])), [x])[1];
		if Sort2([y,z]) in livepairs then
			Add(B.blocks, Sort2([x,y,z]));
		else 
			for i in B.blocks do
				if Size(MultisetIntersection(Sort2([y,z]), i))=2 then
					w:=MultisetDifference(i, Sort2([y,z]))[1];
					B.blocks:=MultisetDifference(B.blocks, [Sort2([w,y,z])]);
					#Error();
					Add(B.blocks, Sort2([x,y,z]));
					break;
				fi;
	...
	od;
	return BlockDesign(B.v, SortListList(B.blocks));
end);
...
BindGlobal("StinsonImproper",function(n)
	...
	while Size(B.blocks) < n*(n-1)/6+1 do
		...
		y:=MultisetDifference(Random(livepairs2), [x])[1];
		z:=MultisetDifference(Random(MultisetDifference(livepairs2, [Sort2([y,x])])), [x])[1];
		if Sort2([y,z]) in livepairs and ( not Sort2([x,y,z]) = [1,2,3]) then
			Add(B.blocks, Sort2([x,y,z]));
		else 
			for i in B.blocks do
				if Size(MultisetIntersection(Sort2([y,z]), i))=2 then
	...
	for j in [1..Length(DigitsNumber(n*(n-1)/6+1,10))+21 ] do 
		Print("\b");
	od;
	B2:=BlockDesign(B.v, SortListList(B.blocks));
